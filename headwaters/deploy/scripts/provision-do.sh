...
DROPLET_NAME="${DROPLET_NAME:-headwaters}"
...
api() {
  ...
  tmp_body="$(mktemp)"
  if [[ -n "$body" ]]; then
    http_code="$(curl -sS -o "$tmp_body" -w '%{http_code}' -X "$method" "https://api.digitalocean.com/v2$path" \
  ...
  if [[ "$http_code" -lt 200 || "$http_code" -ge 300 ]]; then
    echo "digitalocean api $method $path failed with status $http_code" >&2
    cat "$tmp_body" >&2
    rm -f "$tmp_body"
    return 1
  fi
...
existing_key_json="$(api GET "/account/keys" | jq -c --arg name "$SSH_KEY_NAME" '.ssh_keys[] | select(.name == $name)' | head -n 1)"
if [[ -n "$existing_key_json" ]]; then
  ssh_key_id="$(printf '%s' "$existing_key_json" | jq -r '.id')"
else
  if created_key_json="$(api POST "/account/keys" "$(jq -n --arg name "$SSH_KEY_NAME" --arg pk "$SSH_PUBLIC_KEY_CONTENT" '{name:$name, public_key:$pk}')" 2>/dev/null)"; then
    ssh_key_id="$(printf '%s' "$created_key_json" | jq -r '.ssh_key.id')"
  else
    existing_key_json="$(api GET "/account/keys" | jq -c --arg pk "$SSH_PUBLIC_KEY_CONTENT" '.ssh_keys[] | select(.public_key == $pk)' | head -n 1)"
    if [[ -z "$existing_key_json" ]]; then
      echo "failed to create deploy key and could not find an existing matching public key" >&2
      exit 1
    fi
    ssh_key_id="$(printf '%s' "$existing_key_json" | jq -r '.id')"
  fi
...
existing_droplet_json="$(api GET "/droplets?tag_name=$DROPLET_NAME" | jq -c --arg name "$DROPLET_NAME" '.droplets[] | select(.name == $name)' | head -n 1)"
...
if [[ "$USE_RESERVED_IP" == "true" ]]; then
  existing_reserved_json="$(api GET "/reserved_ips" | jq -c '.reserved_ips[] | select(.droplet.id? != null and .droplet.id == '"$droplet_id"')' | head -n 1)"
