/**
 * TCP reference station — entry point.
 *
...
console.log(`tcp-reference-station: starting`);

await space.start();

console.log(`tcp-reference-station: listening (agent: ${space.agentId})`);
if (space.tcpPort) {
...
process.on('SIGINT', async () => {
  console.log('\ntcp-reference-station: shutting down');
  await space.stop();
  process.exit(0);
});
