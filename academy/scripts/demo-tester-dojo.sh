...
case "$AGENT_TARGET" in
  ...
  claude)
    ...
    ;;
  pi)
    run_shell "cd '$REPO_ROOT/intent-space' && set -a && source .env.pi && set +a && cd '$WORKDIR' && npx -y @mariozechner/pi-coding-agent -p --tools read,bash,edit,write,grep,find,ls --provider \"\$PI_PROVIDER\" --model \"\$PI_MODEL\" --api-key \"\$PI_API_KEY\" \"\$(cat tester-prompt.txt)\""
    ;;
  *)
